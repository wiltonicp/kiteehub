		...
		<a-form ref="formRef" :model="formData" :rules="formRules" layout="vertical">
			...
			<a-form-item name="prompt">
				<template #label>
					<a-tooltip>
						<template #title>
							模型固定的引导词，通过调整该内容，可以引导模型聊天方向。该内容会被固定在上下文的开头。可使用变量，例如
							{{ language }}
						</template>
						<question-circle-outlined />
					</a-tooltip>
			...
			<a-form-item name="prologue">
				<template #label>
					<a-tooltip>
						<template #title>
							每次对话开始前，发送一个初始内容。支持标准 Markdown 语法，可使用的额外标记: [快捷按键]:
							用户点击后可以直接发送该问题
						</template>
						<question-circle-outlined />
					</a-tooltip>
...
[快捷按键]: 用户点击后可以直接发送该问题"
			...
			<a-form-item name="searchParam">
				...
				<a-select
					v-model:value="formData.searchParam"
					placeholder="请选择搜索参数"
					:options="knowledgeSearchParametersOptions"
				/>
			</a-form-item>
		</a-form>
		<template #footer>

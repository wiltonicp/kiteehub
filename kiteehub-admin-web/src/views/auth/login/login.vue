	...
	export default {
		...
		data() {
			return {
				...
				captchaOpen: config.SYS_BASE_CONFIG.KITEE_SYS_DEFAULT_CAPTCHA_OPEN,
				validCodeBase64: '',
				ruleForm: {
					account: '',
					password: '',
		...
		mounted() {
			...
			configApi.configSysBaseList().then((data) => {
				if (data) {
					...
					this.captchaOpen = formData.value.KITEE_SYS_DEFAULT_CAPTCHA_OPEN
					tool.data.set('KITEE_SYS_BASE_CONFIG', formData.value)

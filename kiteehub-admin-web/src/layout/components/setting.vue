		...
		methods: {
			changeTopHanderThemeColorOpen() {
				...
				if (!this.topHanderThemeColorOpen) {
					...
					tool.data.set('KITEE_TOP_HANDER_THEME_COLOR_SPREAD', false)
				}
			},
			...
			toggleState(stateName) {
				...
				tool.data.set(`KITEE_${toolDataName}`, this.globalStore[stateName])
			},
			// 设置整体风格主题
			setSideStyle(value) {
				this.globalStore.setTheme(value)
				tool.data.set('KITEE_THEME', value)
			},
			// 设置整体界面布局
			layoutStyle(value) {
				this.globalStore.setLayout(value)
				tool.data.set('KITEE_LAYOUT', value)
			},
			// 切换颜色
			tagColor(value) {
				tool.data.set('KITEE_THEME_COLOR', value)
				this.globalStore.setThemeColor(value)
			},
			// 切换表单风格
			formStyleChange(value) {
				tool.data.set('KITEE_FORM_STYLE', value)

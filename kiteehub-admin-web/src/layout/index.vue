			...
			>
				<header id="snowyHeaderLogo" class="snowy-header-logo">
					<div class="snowy-header-left">
						<div class="logo-bar">
							<img class="logo" :src="sysBaseConfig.KITEE_SYS_LOGO" />
							<span>{{ sysBaseConfig.KITEE_SYS_NAME }}</span>
						</div>
					</div>
				</header>
				...
				<a-layout-content class="main-content-wrapper">
					<div id="adminui-main" class="adminui-main">
						...
						<div class="main-bottom-wrapper">
							<a style="color: #a0a0a0" :href="sysBaseConfig.KITEE_SYS_COPYRIGHT_URL" target="_blank">{{
								sysBaseConfig.KITEE_SYS_COPYRIGHT
							}}</a>
						</div>
					</div>
				</a-layout-content>
			...
			<a-layout-sider v-if="!ismobile" width="80" :theme="sideTheme" :trigger="null" collapsible>
				<header id="snowyHeaderLogo" class="snowy-header-logo">
					<div class="snowy-header-left">
						<div class="logo-bar">
							<router-link to="/">
								<img class="logo" :title="sysBaseConfig.KITEE_SYS_NAME" :src="sysBaseConfig.KITEE_SYS_LOGO" />
							</router-link>
						</div>
					</div>
				</header>
...
<script>
	...
	export default defineComponent({
		...
		created() {
			...
			// 获取缓存中的菜单模块是哪个
			const menuModuleId = tool.data.get('KITEE_MENU_MODULE_ID')
			let menu = []
			if (menuModuleId) {
				// 防止切换一个无此应用的人
